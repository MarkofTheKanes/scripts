...
###################################################################################
###										###
###   UNIMON 1.4 by Stuart.Findlay@cp.net					###
###   ======================================					###
###										###
###   A universal monitoring script						###
###										###
...
monitor_core () {
	...
	do
        	...
        	then
                	...
                	smtptst -spuma.dub0.ie.cp.net f\'core.monitor@cp.net\'r\'$email\'S0Q
        	fi
        	#echo "$control_core"
	done
}
...
get_hpux_top_data() {
	...
	then
		...
		pid=`ssh $host ps -ef | grep java | grep -v 'cp-fs' | awk '{ print $2 }'`
...
get_solaris_top_data() {
	...
	then
		...
		cp processes $workdir
                ...
                do
                        ...
                        nawk -v t=$now_mins '{ printf "%.2f\t%f\n", t, $10 }' $workdir/$host.$processes >> $workdir/$host.$processes.cpu
	...
	then
		...
		ssh $host /usr/local/bin/top -b all | grep $proc | grep -v 'cp-fs' > $workdir/$host.$proc
                nawk -v t=$now_mins '{ printf "%.2f\t%f\n", t, $10 }' $workdir/$host.$proc >> $workdir/$host.$proc.cpu
                nawk -v t=$now_mins '{ str = $6; if (substr(str, length(str)) == "K") {sub(/K/,"",str); i = str/1024 } else {sub(/M/,"",str); i = str } printf "%.2f\t%f\n", t, i }' $workdir/$host.$proc >> $workdir/$host.$proc.mem
        elif [ "$component" = "FS" ]
        then
                # top on host/proc for FS
