Mark O'Kane - 25th May 2017 v1

The script uses bash shell script and as such needs a unix env supporting bash to run it e.g. cygwin if you are on the windows platform.

To run the script do the following on the command line:
	./genDailyBytestotalByAppAndSite-DummyData.sh <no. IMSIs> <start_date_yyyymmddhhmmss> <no. days> e.g.
	./genDailyBytestotalByAppAndSite-DummyData.sh 2 20170601000000 20
	
run the script without arguments to get more details.
...
In /home/boss/bin directory a script to run cqlsh was created
